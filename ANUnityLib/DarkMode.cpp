...
extern "C"
{
	...
	__declspec(dllexport) void SetDarkMode(BOOL useDark, BOOL fixDarkScrollbar);
	__declspec(dllexport) void SetWindowDarkMode(HWND hWnd, BOOL darkMode);

	__declspec(dllexport)
	LRESULT DarkModeWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		//return UAHWndProc(hWnd, message, wParam, lParam, lr, defaultProc) ? TRUE : FALSE;
		
		LRESULT lr;
		if (g_darkModeEnabled && UAHWndProc(hWnd, message, wParam, lParam, &lr, s_DefWndProc))
		{
			return lr;
		}

		return CallWindowProcA(s_DefWndProc, hWnd, message, wParam, lParam);
	}
	...
	void InitDarkModeRoutineOnce(BOOL useDark, BOOL fixDarkScrollbar)
	{
		...
		if (!inited)
		{
			...
			SetWindowDarkMode((HWND) GetMainWindow(), useDark);
			inited = true;
		}
	}
